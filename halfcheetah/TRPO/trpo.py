...
class Policy(nn.Module):
    ...
    def forward(self, states):
        x = self.net(states)
        mean = self.mean(x)
        # Add minimum bound to std
        std = self.log_std.exp().clamp(min=1e-6, max=10)  # Added max bound for stability
        return mean, std
        ...
        for step in range(max_steps):
            try:
                with torch.no_grad():
                    ...
                    # Ensure std is positive and numerically stable
                    std = std.clamp(min=1e-6)
                    dist = Normal(mean, std)
                    # Calculate entropy and ensure it's non-negative
                    entropy = dist.entropy()
                    # The entropy of a normal distribution is:
                    # 0.5 * log(2π * e * σ²), which should always be non-negative for valid σ
                    entropy = entropy.clamp(min=0.0).mean().item()
